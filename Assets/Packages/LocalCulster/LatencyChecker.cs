...
namespace LocalClustering
{
    ...
    public class LatencyChecker : MonoBehaviour
    {
        ...
        public void Start()
        {
            ...
            LocalClusterNetworkManager.singleton._OnServerDisconnect += (conn) =>
            {
            ...
            LocalClusterNetworkManager.singleton._OnStartClient += () =>
            {
                ...
                {
                    LocalCluster.client.RegisterHandler(CustomMsgType.Latency, (msg) =>
                    {
#if INCLUDE_UPDATE
        ...
        public void Update()
        {
            ...
            if (LocalCluster.isSlaver)
            {
                if (_lastMsg != null)
                {
                    LocalCluster.client.Send(CustomMsgType.Latency, _lastMsg);
                    _lastMsg = null;
                }
            }
#endif      
